...
public class BasicSecurityConfig extends WebSecurityConfigurerAdapter{
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception{
		...
		auth.inMemoryAuthentication()
			...
			.password(senhaEncoder().encode("root")) // --> Define a senha do usuario
	...
	protected void configure(HttpSecurity http) throws Exception{
		...
		http.authorizeRequests() // --> Sessão para autorização do usuario
			.antMatchers("/usuario/login").permitAll() // --> Permite com que todos tenham acesso ao login
			.antMatchers("/usuario/registro").permitAll() // --> Permite com que todo tenham acesso ao cadastro
